...
func validateCommon(args commonArgs) error {
	...
	case "1024x1024", "1536x1024", "1024x1536":
		return nil
	default:
		return errors.New("size must be one of 1024x1024, 1536x1024, or 1024x1536")
	}
}
...
func apiKey() (string, error) {
	...
	if key == "" {
		return "", errors.New("GROK_API_KEY is not set; set it locally, then retry")
	}
	return key, nil
}
...
func generate(prompt, out string, args commonArgs) (map[string]any, error) {
	...
	ep := endpoint(os.Getenv("GROK_API_URL"), "generations")
...
func edit(prompt string, imagePaths []string, mask, out string, args commonArgs) (map[string]any, error) {
	return nil, errors.New("grok-imagine-image-quality does not support image-editing / inpaint / mask operations. Only text-to-image generation (generate / generate-batch) is available.")
}
...
func parseCommon(fs *flag.FlagSet, argv []string, args *commonArgs) error {
	...
	fs.StringVar(&args.size, "size", defaultSize, "one of 1024x1024, 1536x1024, or 1024x1536")
	...
	for range concurrency {
		...
		go func() {
			...
			for index := range queue {
				...
				if strings.EqualFold(job.Operation, "edit") || strings.EqualFold(job.Operation, "inpaint") || strings.EqualFold(job.Operation, "mask") || job.Image != "" || len(job.Images) > 0 || job.Mask != "" {
					results[index] = result{Index: index + 1, OK: false, Error: "grok-imagine-image-quality does not support image-editing / inpaint / mask operations. Only text-to-image generation (generate / generate-batch) is available."}
					mu.Lock()
					failed = true
					mu.Unlock()
					continue
...
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: grok-image <generate|generate-batch|edit> [options]")
}
